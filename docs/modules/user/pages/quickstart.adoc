...
====
When creating your docker-compose.yml don't forget to modify the volume mappings for `/run/udev` and `/dev` path, as we mounted it on `/mnt/udev` and `/mnt/dev` inside our LXC
====
...
# 8BitDo Ultimate 2 virtual pad: allow Steam/user access to hidraw
SUBSYSTEM=="hidraw", KERNEL=="hidraw*", KERNELS=="0003:2DC8:6012.*", \
  ENV{ID_BUS}="usb", ENV{ID_VENDOR_ID}="2dc8", ENV{ID_MODEL_ID}="6012", \
  ENV{HID_ID}="0003:00002DC8:00006012", ENV{HID_NAME}="8BitDo Ultimate 2 Wireless Controller for PC", \
...
SUBSYSTEMS=="input", ATTRS{name}=="8BitDo Ultimate 2 Wireless Controller for PC", MODE="0660", ENV{ID_SEAT}="seat9"
....
